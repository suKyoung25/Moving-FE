...
export default function SupportInput({ name, label, important, textarea, fileupload, validate, fileValidate, setValidationState }: SupportInputProps) {
    // 텍스트 값 상태
    ...
    const commonProps = {
        ...
        className: `w-full pt-0 pb-1.5 md:pb-3 border-b ${isFocused ? "border-primary-blue-300" : "border-gray-400"
            } transition-all duration-300`
    };

    return (
        <div className={`w-full pt-2 md:pt-4 text-14-medium md:text-18-medium ${errorMsg && "[&_*]:text-red-500 [&_*]:border-red-500"}`}>
            ...
            <div
                className={`
                    ${(!isFocused || errorMsg) ? "opacity-100 text-red-500" : "opacity-0"}
                    min-h-5 md:min-h-7 mt-1.5 text-12-medium md:text-16-medium
