   ...
   ({
      ...
      const getMoveTypeLabel = (moveType: string) => {
         const map: Record<string, string> = {
            SMALL: t("moveTypes.SMALL"),
            OFFICE: t("moveTypes.OFFICE"),
            HOME: t("moveTypes.HOME"),
         };
         ...
         <li className="border-b-line-200 mx-4 space-y-4 border-b py-8 text-sm font-normal lg:space-y-6 lg:text-lg">
            ...
            {isMoverView && (
               <div className="flex gap-2 text-xs text-gray-400">
                  <span>
                     {getMoveTypeLabel((review as MoverReview).moveType)}
                  </span>
                  <LineDivider className="mx-1 flex h-3 w-0.5 flex-col" />
                  <span>
                     {(review as MoverReview).price.toLocaleString()}
                     {t("money")}
                  </span>
